            ...
            <div class="col-md-12 col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <span class="float-left"><h4> Attendance Report For {{$student->name}}</h4></span>
                    </div>
                    <div class="card-body">
                        @if(session('success'))
                            ...
                            @if ($student->courses->count() > 0)
                                @foreach($student->courses as $ck => $course)
                                    ...
                                    <div class="p-0 mx-auto d-flex info">
                                        ...
                                        <p class="w-25"><i class="fa fa-check-circle"></i> <span> Phone: </span>
                                            {{$student->phone}}
                                        </p>
                                        <p class="w-25"><i class="fa fa-check-circle"></i> <span> Course: </span>
                                            {{$course->title}}
                                        </p>
                                    </div>
                                    <div class="p-0 mx-auto d-flex info">
                                        <p class="w-25"><i class="fa fa-check-circle"></i><span> Batch: </span>
                                            @foreach($student->batches as $bk => $batch)
                                            {{-- {{dd($batch)}} --}}
                                            {{batch_name($batch->course->title_short_form, $batch->year, $batch->month, $batch->batch_number)}}
                                            @endforeach
                                        </p>
                                        <p class="w-25"><i class="fa fa-check-circle"></i> <span> Total Class: </span>
                                            @foreach($student->batches as $bk => $batch)
                                        ...
                                        <p class="w-25"><i class="fa fa-check-circle"></i> <span> Total Present: </span>
                                            @foreach($student->batches as $bk => $batch)
                                                @foreach($minfo as $infos)
                                                    @foreach($infos as $info)
                                                        @if($course->id == $info->course_id && $batch->id == $info->batch_id)
                                        ...
                                        <p class="w-25"><i class="fa fa-check-circle"></i> <span> Total Absent: </span>
                                            @foreach($student->batches as $bk => $batch)
                                                @foreach($minfo as $infos)
                                                    @foreach($infos as $info)
                                                        @if($course->id == $info->course_id && $batch->id == $info->batch_id)
